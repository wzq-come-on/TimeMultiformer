...
def train(model, train_dataloader, val_dataloader, optimizer):
    ...
    for epoch in range(args.epochs):
        ...
        if best_ROCAUC < classification_metrics["ROC_AUC"]:
            ...
            # 添加提示信息
            logger.info(f"在 epoch {epoch} 时保存模型，当前最佳 ROC_AUC 为 {best_ROCAUC:.4f}，保存路径为 {saving_path}")
        else:
            current_patience -= 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", type=str,default = "logs/TimeMultiformer/AUC", help="model and log saving dir")
    parser.add_argument(
        "--original_dataset_path", type=str,default = "data/physio2012_37feats_01masked/datasets.h5" ,help="path of original dataset"
    )
    parser.add_argument(
        "--imputed_dataset_path", type=str,default = "data/physio2012_37feats_01masked/imputations.h5" ,help="path of imputed dataset"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default = True,
        help="train_mode choose False and test_mode choose True",
    )
    parser.add_argument(  
        "--saved_model_path",  
        type=str,  
        default="./logs/TimeMultiformer/AUC/models/2025-05-02_T18-01-05/model_epoch_15_ROCAUC_0.8557",
