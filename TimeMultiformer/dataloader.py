...
def normalize_3d_array(array):
    ...
    for i in range(array.shape[2]):
        ...
        normalized_array[:, :, i] = normalized_2d
...
def parse_deltaPre(X, masks, seq_len):
    deltaPre = []  
    time_seq=list(range(seq_len))
    for i in range(len(X)):
        t_deltaPre = [0.0] * len(X[i])
        deltaPre.append(t_deltaPre)
        if i == 0:
            continue
        for j in range(len(X[i])):
            if masks[i - 1][j] == 1:
                deltaPre[i][j] = time_seq[i] - time_seq[i - 1]
            if masks[i - 1][j] == 0:
                deltaPre[i][j] = time_seq[i] - time_seq[i - 1] + deltaPre[i - 1][j]
    return np.asarray(deltaPre)

def parse_H(masks, hint_rate):
    unif_random_matrix = np.random.uniform(0., 1., size=masks.shape)
    binary_random_matrix = 1 * (unif_random_matrix < hint_rate)
    H = masks * binary_random_matrix
    return H
...
class LoadValTestDataset(LoadDataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.model_type in ['TimeMultiformer']:
            deltaPre = parse_deltaPre(self.X_hat[idx],self.missing_mask[idx],self.seq_len)
            sample = (
                torch.tensor(idx),
                torch.from_numpy(self.X_hat[idx].astype('float32')),
                torch.from_numpy(self.missing_mask[idx].astype('float32')),
                torch.from_numpy(deltaPre.astype('float32')),
                torch.from_numpy(self.X[idx].astype('float32')),
                torch.from_numpy(self.indicating_mask[idx].astype('float32')),
            )
        else:
...
class LoadTrainDataset(LoadDataset):
    ...
    def __init__(self, file_path, seq_len, feature_num, model_type, masked_imputation_task, miss_rate):
        ...
        if masked_imputation_task:
            self.artificial_missing_rate = miss_rate
    ...
    def __getitem__(self, idx):
        ...
        if self.masked_imputation_task:
            ...
            if self.model_type in ['Transformer', 'SAITS']:
                ...
                deltaPre = parse_deltaPre(X_hat,missing_mask,self.seq_len)
                sample = (
                    torch.tensor(idx),
                    torch.from_numpy(X_hat.astype('float32')),
                    torch.from_numpy(missing_mask.astype('float32')),
...
class LoadDataForImputation(LoadDataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.model_type in ['AttentionGAN']:
            sample = (
                torch.tensor(idx),
                torch.from_numpy(self.X[idx].astype('float32')),
                torch.from_numpy(self.missing_mask[idx].astype('float32'))
...
class UnifiedDataLoader:
    def __init__(self, dataset_path, seq_len, feature_num, model_type,miss_rate, batch_size=1024, num_workers=4,
    ...
    def get_train_val_dataloader(self):
        self.train_dataset = LoadTrainDataset(self.dataset_path, self.seq_len, self.feature_num, self.model_type,
                                              self.masked_imputation_task,self.miss_rate)
